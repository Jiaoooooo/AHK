...
^w::
{
ControlClick,Menu Bar,HqPDS,,,,NA x250 y10
Return
}
...
if (controlName == CurrControlName)
	Return True
else
	Return False
}
...
!WheelDown::
{
	...
	{
		SendMessage, 0x1330, 1,, SysTabControl321, 模板
		Sleep,100
		ControlClick,Button1,模板,,,,NA
		WinWait,打开,,1
		if ErrorLevel
		{
			Send,{Enter}
			...
			if ErrorLevel
			{
				MsgBox,未能打开文件窗口,请手动打开
				Sleep,3000
			}
		}
		ControlFocus,DirectUIHWND2,打开
		;Send,{down}{Up}
		Sleep,500
		WinWaitClose,打开
		Sleep,100
		Loop
		{
			ControlGetText,NewModuleAddress,Edit1,模板
			if (NewModuleAddress <> "")
				Break
			IfWinNotExist ,模板
				Break
		...
		;ControlGetText,NewModuleAddress,Edit1,模板
		WriteModuleAddressToIni(NewModuleAddress)						; 将当前调用的模块地址保存进配置文件
		ControlFocus,确定,模板,,,,NA
	}
	Return
}
...
WriteModuleAddressToIni(EditText)
{
if (EditText <> "")
	IniWrite, %EditText%, %HQ_INI_ADD%, SETTING, LAST_MODULE_ADDRESS		; 将刚才读取的模块地址保存的配置文件
Return
}

ReadModuleAddressFromIni()
{
IniRead, IniStr, %HQ_INI_ADD%, SETTING, LAST_MODULE_ADDRESS,0				; 从配置文件中读取上一个模块的地址
if (IniStr <> 0)
	Return %IniStr%
else
	return
...
!F1::
{
ControlClick,TPanel14,ahk_class TMainForm,,,,NA x48 y17
Return
}

/*
Global isPatternExpanded := 0
^`::
{
if !isPatternExpanded
	{
	ControlClick,TPanel66,ahk_class TMainForm,,,,NA x80 y14
	isPatternExpanded := 1
	}
else{
	ControlClick,TPanel66,ahk_class TMainForm,,,,NA x146 y14
	isPatternExpanded := 0
}
Return
}
